-- =====================================================
-- FUNCIÓN DE TRANSFERENCIA FINAL CORREGIDA
-- =====================================================
-- Maneja correctamente las unidades LEAP vs atómicas
-- =====================================================
...
BEGIN
    ...
    SELECT COALESCE(token_balance, 0), COALESCE(full_name, email) 
    ...
    IF sender_balance < 10000000 THEN
        sender_balance := sender_balance * 1000000000;
    END IF;
    
    IF receiver_balance < 10000000 THEN
        receiver_balance := receiver_balance * 1000000000;
    END IF;
    ...
    BEGIN
        ...
        INSERT INTO token_transactions (
            id, user_id, amount, type, description, reference_type, created_at
        ) VALUES (
            gen_random_uuid(), sender_id, 1, 'transfer', 'test', 'test', NOW()
        );
    ...
    EXCEPTION WHEN OTHERS THEN
        BEGIN
            ...
            INSERT INTO token_transactions (
                ...
                gen_random_uuid(), sender_id, 1, 'transaction', 'test', 'test', NOW()
            );
            DELETE FROM token_transactions WHERE description = 'test' AND reference_type = 'test';
        ...
        EXCEPTION WHEN OTHERS THEN
            BEGIN
                ...
                INSERT INTO token_transactions (
                    ...
                    gen_random_uuid(), sender_id, 1, 'payment', 'test', 'test', NOW()
                );
                DELETE FROM token_transactions WHERE description = 'test' AND reference_type = 'test';
                valid_type := 'payment';
            EXCEPTION WHEN OTHERS THEN
                BEGIN
                    ...
                    INSERT INTO token_transactions (
                        ...
                        gen_random_uuid(), sender_id, 1, 'send', 'test', 'test', NOW()
                    );
                    DELETE FROM token_transactions WHERE description = 'test' AND reference_type = 'test';
                    valid_type := 'send';
                EXCEPTION WHEN OTHERS THEN
    ...
    BEGIN
        ...
        BEGIN
            IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_balances') THEN
                ...
                SET balance = balance - amount_atomic,
                    updated_at = NOW()
...
BEGIN
    RAISE NOTICE '==============================================';
    RAISE NOTICE 'FUNCIÓN DE TRANSFERENCIA CORREGIDA INSTALADA';
    RAISE NOTICE 'Ahora maneja correctamente las unidades LEAP vs atómicas';
    RAISE NOTICE '==============================================';
END $$;
