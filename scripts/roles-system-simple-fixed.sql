-- =====================================================
-- SISTEMA DE ROLES SIMPLE Y DIRECTO - VERSIÓN CORREGIDA
-- 1. Eliminar dependencias primero
-- 2. Crear tabla roles
-- 3. Cambiar users.role por users.role_id
-- 4. Facilitadores simples
-- =====================================================

-- 1. ELIMINAR TRIGGERS Y FUNCIONES QUE DEPENDEN DE LA COLUMNA ROLE
DROP TRIGGER IF EXISTS on_user_role_change ON users;
DROP FUNCTION IF EXISTS handle_user_role_change();
DROP FUNCTION IF EXISTS update_facilitator_on_role_change();
...
-- 4. AGREGAR COLUMNA ROLE_ID PRIMERO (antes de eliminar role)
ALTER TABLE users 
ADD COLUMN IF NOT EXISTS role_id UUID REFERENCES roles(id);

-- 5. MIGRAR DATOS DE ROLE A ROLE_ID
UPDATE users 
SET role_id = (
  SELECT r.id 
  FROM roles r 
  WHERE r.name = COALESCE(users.role, 'user')
)
WHERE role_id IS NULL;

-- 6. ASIGNAR ROL POR DEFECTO A USUARIOS SIN ROL
UPDATE users 
SET role_id = (SELECT id FROM roles WHERE name = 'user')
WHERE role_id IS NULL;

-- 7. AHORA SÍ ELIMINAR COLUMNA ROLE CON CASCADE
ALTER TABLE users DROP COLUMN IF EXISTS role CASCADE;

-- 8. HACER ROLE_ID OBLIGATORIO
ALTER TABLE users 
ALTER COLUMN role_id SET NOT NULL;

-- 9. CREAR ALGUNOS USUARIOS FACILITADORES
...
-- 10. LIMPIAR Y RECREAR TABLA FACILITATORS
...
-- 11. CREAR TRIGGER PARA UPDATED_AT EN FACILITATORS
...
-- 12. CREAR FACILITADORES PARA USUARIOS CON ROL FACILITATOR
...
-- 15. FUNCIÓN AUXILIAR PARA OBTENER ROL DE USUARIO
...
-- 16. RECREAR TRIGGER PARA NUEVOS USUARIOS (ACTUALIZADO PARA USAR ROLE_ID)
...
BEGIN
  ...
  VALUES (
    ...
    2450,
    (SELECT id FROM roles WHERE name = 'user') -- Rol por defecto
  );
  RETURN NEW;
END;
...
DO $$
BEGIN
    RAISE NOTICE '✅ ¡SISTEMA DE ROLES SIMPLE CREADO CORRECTAMENTE!';
    RAISE NOTICE '🗑️  Triggers problemáticos eliminados';
    RAISE NOTICE '📋 Tabla roles: user, facilitator, admin';
    RAISE NOTICE '🔗 users.role_id -> roles.id';
    RAISE NOTICE '👥 Facilitadores creados para usuarios con rol facilitator';
    RAISE NOTICE '📅 Eventos creados con facilitadores reales';
    RAISE NOTICE '🚀 ¡Sistema funcional y sin dependencias problemáticas!';
END $$;
