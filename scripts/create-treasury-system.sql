-- Crear sistema de Treasury para tokens LEAP
...
CREATE TABLE public.atomic_token_operations (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Insertar cuentas del Treasury
INSERT INTO treasury_accounts (account_type, name, description, balance) VALUES
('issuer', 'LEAP Token Issuer', 'Cuenta emisora principal de tokens LEAP', 1000000000000), -- 1M LEAP en unidades atómicas
('platform', 'MyLeap Platform', 'Cuenta de la plataforma para operaciones', 0),
('reserve', 'Reserve Fund', 'Fondo de reserva para estabilidad', 0),
('rewards', 'Rewards Pool', 'Pool de recompensas para usuarios', 0);
...
-- Función para procesar operación atómica
CREATE OR REPLACE FUNCTION process_atomic_operation(
  p_operation_type TEXT,
  p_from_account_type TEXT DEFAULT NULL,
  p_from_account_id UUID DEFAULT NULL,
  p_to_account_type TEXT DEFAULT NULL,
...
-- Función para transferir tokens entre usuarios (mejorada)
...
-- Función para comprar curso (nueva)
...
-- Función para registrar evento (nueva)
...
-- Función para dar recompensas (nueva)
...
CREATE INDEX idx_atomic_operations_status ON atomic_token_operations(status);
CREATE INDEX idx_atomic_operations_type ON atomic_token_operations(operation_type);
CREATE INDEX idx_atomic_operations_from ON atomic_token_operations(from_account_type, from_account_id);
CREATE INDEX idx_atomic_operations_to ON atomic_token_operations(to_account_type, to_account_id);
CREATE INDEX idx_atomic_operations_created ON atomic_token_operations(created_at);
...
-- Solo admins pueden ver treasury
...
-- Función para obtener balance de usuario (mejorada)
