-- =====================================================
-- ARREGLAR DEPENDENCIAS DEL SISTEMA DE ROLES
...
-- 7. Crear algunos usuarios facilitadores de ejemplo
...
-- 9. Crear la función correcta para manejar cambios de rol
...
-- 10. Crear el trigger que funciona con role_id
CREATE TRIGGER on_user_role_change
  AFTER INSERT OR UPDATE OF role_id ON users
  FOR EACH ROW EXECUTE FUNCTION create_facilitator_from_role();

-- 11. Crear facilitadores para usuarios existentes que ya tienen role_id de facilitator
...
-- 12. Actualizar función get_upcoming_events
...
-- 13. Limpiar eventos y crear nuevos
DELETE FROM events;
...
-- 15. Crear trigger para updated_at en roles
...
DO $$
BEGIN
    RAISE NOTICE '✅ ¡DEPENDENCIAS ARREGLADAS Y SISTEMA RECREADO!';
    RAISE NOTICE '🗑️ Triggers antiguos eliminados correctamente';
    RAISE NOTICE '📋 Tabla roles creada con estructura normalizada';
    RAISE NOTICE '🔗 Usuarios conectados via role_id';
