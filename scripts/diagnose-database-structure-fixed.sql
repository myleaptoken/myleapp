...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'token_transactions') THEN
        ...
        -- Contar registros
        SELECT COUNT(*) INTO trans_count FROM token_transactions;
        RAISE NOTICE '  📊 Total transacciones: %', trans_count;
    ELSE
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_balances') THEN
        ...
        RAISE NOTICE '  📊 Total balances: %', balance_count;
    ELSE
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'treasury_accounts') THEN
        ...
        RAISE NOTICE '  📊 Total cuentas treasury: %', treasury_count;
    ELSE
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'atomic_token_operations') THEN
        ...
        -- Contar registros
        SELECT COUNT(*) INTO atomic_count FROM atomic_token_operations;
        RAISE NOTICE '  📊 Total operaciones atómicas: %', atomic_count;
    ELSE
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users') THEN
        ...
        FOR user_rec IN 
            ...
            LIMIT 5
        LOOP
            RAISE NOTICE '  👤 %: % (% tokens)', user_rec.email, user_rec.name, user_rec.balance;
        END LOOP;
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'token_transactions') THEN
        ...
        FOR trans_rec IN 
            ...
            LIMIT 3
        LOOP
            RAISE NOTICE '  💸 ID %: % tokens (%)', trans_rec.id, trans_rec.amount, trans_rec.transaction_type;
        END LOOP;
    END IF;
END $$;
