-- =====================================================
-- CREATE WORKING TRANSFER FUNCTION
-- =====================================================
-- Uses discovered valid types for token_transactions
-- =====================================================
...
DECLARE
    ...
    operation_id UUID;
    valid_type TEXT := 'transfer'; -- Default to transfer
    result JSON;
BEGIN
    ...
    -- Try to determine valid type for token_transactions
    -- Test common values in order of preference
    BEGIN
        INSERT INTO token_transactions (
            id, user_id, amount, type, description, created_at
        ) VALUES (
            gen_random_uuid(), sender_id, -1, 'transfer', 'Test', NOW()
        );
        DELETE FROM token_transactions WHERE amount = -1 AND description = 'Test';
        valid_type := 'transfer';
    EXCEPTION WHEN check_violation THEN
        BEGIN
            INSERT INTO token_transactions (
                ...
                gen_random_uuid(), sender_id, -1, 'payment', 'Test', NOW()
            );
            DELETE FROM token_transactions WHERE amount = -1 AND description = 'Test';
            valid_type := 'payment';
        EXCEPTION WHEN check_violation THEN
            BEGIN
                INSERT INTO token_transactions (
                    ...
                    gen_random_uuid(), sender_id, -1, 'credit', 'Test', NOW()
                );
                DELETE FROM token_transactions WHERE amount = -1 AND description = 'Test';
                valid_type := 'credit';
            EXCEPTION WHEN check_violation THEN
                valid_type := 'transaction'; -- Last resort
            END;
        END;
    END;
    ...
    -- 2. Record sender transaction (negative amount)
    INSERT INTO token_transactions (
        id,
        user_id,
        amount,
    ...
    -- 3. Record receiver transaction (positive amount)
    INSERT INTO token_transactions (
        id,
        user_id,
        amount,
